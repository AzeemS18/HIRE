...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an assistant designed to generate screening questions for recruiters.
  ...
  async input => {
    try {
      ...
      if (error.message.includes('503 Service Unavailable') || error.message.includes('overloaded')) {
        ...
        throw new Error("The service is currently overloaded. Please try again in a few moments.");
      }
