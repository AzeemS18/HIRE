 ...
 *
 * - xyroChat - A function that takes a message and returns a response.
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are Xyro, a powerful, helpful, and friendly assistant, similar to Gemini, designed for the HireGenius platform. Your goal is to provide intelligent and helpful responses to a wide variety of user questions.
  ...
  async input => {
    try {
      ...
      if (error.message.includes('503 Service Unavailable') || error.message.includes('overloaded')) {
        return { response: "The service is currently experiencing high demand, which may cause it to be temporarily unavailable. Please try again in a few moments." };
      }
      // Re-throw other errors to be caught by the client-side handler
      throw new Error("An unexpected error occurred while communicating with the service.");
    }
  }
);
